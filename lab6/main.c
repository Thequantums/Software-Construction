...
//global variables
int nthreads; // store number of threads
scene_t scene;
float arrcolor[width][height][3];
...
void *thread(void *vargp) {
  ...
  for( int px = ipx; px<width; px = px + nthreads )
  {
	...
	mul( pixel_color, pixel_color,
      ...
      arrcolor[px][py][0] = gamma( pixel_color[0] ) * max_color;
      arrcolor[px][py][1] = gamma( pixel_color[1] ) * max_color;
      arrcolor[px][py][2] = gamma( pixel_color[2] ) * max_color;

      /* enforce caps, replace with real gamma */
      for( int i=0; i<3; i++)
	arrcolor[px][py][i] = max( min(arrcolor[px][py][i], 255), 0);
...
main( int argc, char **argv )
{
	...
	arri[i] = i;
	validate = pthread_create(&tid[i], NULL, thread, &arri[i]);
      if (validate){
	fprintf( stderr, "Cant create a thread error." );
	exit(1);
      }
    }
    ...
    for (i = 0; i < nthreads; i++){
      ...
      validate = pthread_join(tid[i], NULL);
      if (validate){
	fprintf( stderr, "Cant wait for a thread to join error!");
	exit(1);
      }
    }
    ...
    for (a = 0; a < width; a++)
    {
      ...
      {
	printf( "%.0f %.0f %.0f\n",
		arrcolor[a][b][0], arrcolor[a][b][1], arrcolor[a][b][2] );
      }
      printf( "\n" );
    }
